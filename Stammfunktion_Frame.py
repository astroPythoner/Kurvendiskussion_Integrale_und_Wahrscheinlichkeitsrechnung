...
def make_stammfunktion(parameter,funktion,row,frame,print_stammfunktion=True):
    ...
    if not "x" in funktion.funktion_user_kurz:
        ...
        tk.Label(frame, text="F(x) = "+stammfunk.funktion_user_kurz).grid(row=row+2, column=1)
        ...
        tk.Label(frame, text="F(x) = "+stammfunktion_lang).grid(row=row+2, column=1)
        tk.Label(frame, text="F(x) = "+stammfunk.funktion_user_kurz).grid(row=row+3, column=1)
        ...
        if funktion.trigonometrisch_d != 0:
            ...
            tk.Label(frame, text="F(x) = " + funktion.funktion_trigonometrisch_x_ersetzbar+"x").grid(row=row+1, column=1)
        ...
        if funktion.trigonometrische_funktion == "sin":
            ...
            tk.Label(frame, text="F(x) = " + str(funktion.trigonometrisch_a) + " * -cos(" + n_mal_x_plus_m_to_string(funktion.trigonometrisch_b, -funktion.trigonometrisch_c) + ") / "+str(funktion.trigonometrisch_b)+" "+stammfunktion_ende).grid(row=row+row_add+1, column=1)
            ...
            tk.Label(frame, text="F(x) = " + str(funktion.trigonometrisch_a) + " * sin(" + n_mal_x_plus_m_to_string(funktion.trigonometrisch_b, -funktion.trigonometrisch_c) + ") / "+str(funktion.trigonometrisch_b)+" "+stammfunktion_ende).grid(row=row+row_add+1, column=1)
        ...
        tk.Label(frame, text="F(x) = "+stammfunk.funktion_user_kurz).grid(row=row+row_add+2, column=1)
    elif funktion.is_wurzel:
        stammfunktion_ende = ""
        row_add = 0
        if funktion.wurzel_d != 0:
            tk.Label(frame, text=" konstante Zahl mit x erweitern").grid(row=row+0, column=2,sticky = tk.W)
            tk.Label(frame, text="F(x) = " +funktion.funktion_wurzel_x_ersetzbar+"x").grid(row=row+1, column=1)
        ...
        tk.Label(frame, text="F(x) = "+str(funktion.wurzel_a)+"* (2/3)*("+n_mal_x_plus_m_to_string(funktion.wurzel_b,-funktion.wurzel_c)+")'(3/2) / "+str(funktion.wurzel_b)+" "+stammfunktion_ende).grid(row=row+row+1, column=1,sticky=tk.W)
        ...
        try:
            ...
            if funktion_erkannt:
                tk.Label(frame, text="F(x) = " + stammfunk.funktion_user_kurz).grid(row=row+1, column=1)
            else:
    ...
    if print_stammfunktion:
        tk.Label(frame, text="Stammfunktion: F(x) = " + stammfunk.funktion_user_kurz).grid(row=row+4, column=0, sticky=tk.W, columnspan=2)
    return stammfunk,row
...
class Stammfunktion_Frame(tk.Frame):
    ...
    def stammfunktion_bestimmen(self):
        stammfunk,row = make_stammfunktion(self.parameter,self.__funktion,0,self)
